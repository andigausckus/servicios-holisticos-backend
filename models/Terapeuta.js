...
const TerapeutaSchema = new mongoose.Schema({
  ...
  modalidad: { type: String, enum: ["Online", "Presencial", "Ambas"] },
  fotoPerfil: String,
  fotoPortada: String,
  ...
  cbuCvu: {
    ...
    validate: {
      validator: function (v) { return /^\d{22}$/.test(v); },
  ...
  bancoOBilletera: {
    ...
    enum: [
      "Mercado Pago", "Ualá", "Naranja X", "Brubank", "Banco Nación", 
      "Banco Provincia", "Banco Galicia", "Banco Santander", "Banco BBVA", "Banco Macro"
    ]
  },

  aprobado: { type: Boolean, default: true },
  disponibilidad: { type: [disponibilidadSemanaSchema], default: [] },
  disponibilidadPorFechas: { type: [disponibilidadFechaSchema], default: [] },
  horariosBloqueados: {
    type: [
      {
  ...
  // 🔹 Aquí cambiamos a referencias de servicios
  servicios: [{ type: mongoose.Schema.Types.ObjectId, ref: "Servicio" }]
}, { timestamps: true, toJSON: { virtuals: true }, toObject: { virtuals: true } });

// Virtual para calcular promedio de reseñas de todos sus servicios
TerapeutaSchema.virtual('puntuacionPromedio').get(function () {
  let totalPuntuacion = 0;
  let totalReseñas = 0;

  if (Array.isArray(this.servicios) && this.populated('servicios')) {
    this.servicios.forEach(servicio => {
      if (Array.isArray(servicio.resenas)) {
        servicio.resenas.forEach(resena => {
          totalPuntuacion += resena.puntaje || 0;
          totalReseñas += 1;
        });
      }
    });
  ...
  return totalReseñas === 0 ? 0 : (totalPuntuacion / totalReseñas).toFixed(1);
});
